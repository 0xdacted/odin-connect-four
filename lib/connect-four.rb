  ...
  def winner?(name, symbol, board = @board)
    board.each_with_index do |row, row_index|
      row.each_with_index do |_piece, column_index|
        if !(row_index > 2) && (board[row_index][column_index] == symbol && board[row_index + 1][column_index] == symbol && board[row_index + 2][column_index] == symbol && board[row_index + 3][column_index] == symbol)
          puts "Congratulations #{name}, you have won!"
          return true
        end
        if !(column_index > 3) && (board[row_index][column_index] == symbol && board[row_index][column_index + 1] == symbol && board[row_index][column_index + 2] == symbol && board[row_index][column_index + 3] == symbol)
          puts "Congratulations #{name}, you have won!"
          return true
        end
        next if row_index > 2 || column_index > 3
...
def player_one_turn
  ...
  player_one_turn if column == false
...
def player_two_turn
  ...
  player_two_turn if column == false
  if place_symbol(column, symbol) == false
    player_two_turn
  else
    winner?(name, symbol)
